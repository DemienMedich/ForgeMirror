...
- Внутри каталога хранения:
  - профили: `*.json`
  - архив: `archive/*.json`
  - правила: `meta/gameplay.json`
  - каталог навыков: `skills.json`
  - пароль администратора: `meta/admin.json` (или `FORGEMIRROR_ADMIN_PASSWORD`, также `JOBSKILL_ADMIN_PASSWORD`)
  - layout GUI: `meta/gui-layout.ini`

Профили лежат вне папок сборки (`build/`), поэтому прогресс переживает пересборки. Legacy-файлы `.ini`/`.txt` автоматически мигрируют в JSON и удаляются после переноса.
...
## Настройка правил XP
- Глобальные параметры прокачки живут в `meta/gameplay.json` в каталоге хранения (создаётся автоматически). CLI и GUI читают эти значения при запуске.
...
- Навыки хранятся в `skills.json` в каталоге хранения.
- Пароль администратора хранится в `meta/admin.json` (можно переопределить через `FORGEMIRROR_ADMIN_PASSWORD`, также `JOBSKILL_ADMIN_PASSWORD`).
- Архивные профили лежат в `archive/`.
- Профили - JSON-файлы (`0001.json`, `0002.json`, ...); ID увеличиваются автоматически.

## Идеи для Roadmap
- Версионирование JSON-форматов и инструменты миграции.
