...
| Area | Qt coverage through stage 87 | Remaining |
| --- | --- | --- |
...
The manual per-file conflict dialog covers tasks, pipeline and projects. Each local/cloud replacement requires an explicit confirmation; local and cloud preimages are saved in `meta/updates`, writes are atomic, and source/destination snapshots are rechecked before the cloud file is replaced. The shared backup browser now recognizes project snapshots as well. This remains a three-file operation, not a whole-workspace push; automatic sync remains disabled.
...
Stage 45 push is manual and limited to tasks/pipeline; stage 87 adds projects to the same individually confirmed comparison and backup flow. Automatic pull/push, timers, background conflict decisions and snapshot pruning remain disabled.
