...
| Area | Qt coverage through stage 98 | Remaining |
| --- | --- | --- |
| Profiles | Selector, compact level/XP/task metrics, skills, transactional direct skill/global XP grant paired with profile audit, transactional admin edit of name/profession/spirit/block state paired with profile audit, archive/restore and password reset/change paired with audit in recovery journals, guarded permanent deletion of empty archived profiles, rename with stable ID, session or 30/90-day local trust with transactional audited login/logout, achievement viewing/granting/editing/revocation and local icons; wallet balance and admin adjustment, Pomodoro reward, personal evil-spirit removal, all paired with wallet audit in a recovery journal; wallet history, read-only profile audit and task/XP history including awarded and pending tasks | Trust pruning and stale-access invalidation audit edge cases |
