...
- State: Stage 9 profession editors and skill bindings implemented and verified. Shared loader fixes are confined to migration branch; stable branches remain untouched.
...
- Now: Stage 9 verified: atomic profession creation/editing, skill profession multi-selection, unknown-link preservation, catalog profession display/search. Fixed category+profession token parsing and profession first-ID BOM handling. smoke_qt/smoke_core pass; both forms visually inspected; native tests and packaged startup without Qt on PATH exit 0, stderr empty. Commit/push to codex/qt-gui.
- Next: Continue Qt migration: achievements and personal wallet/Pomodoro actions, persistent trusted-device access and profile audit, destructive operations with rollback, settings, cloud, vault and 3D. Existing malformed cross-file IDs are not rewritten automatically; unknown references are preserved for review.
...
- Goal (incl. success criteria):
  ...
  - Product adoption roadmap: превратить ForgeMirror из экспериментальной геймификации в управленческий инструмент с доказуемой пользой: отчёты по задачам/проектам/XP, устойчивое ядро, понятные сценарии работы и экспорт для руководителя.
