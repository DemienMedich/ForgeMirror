...
- State:
  - Ветка: develop. Версия: 0.4.93.
  - Последнее изменение: продолжен общий UX pass, пункт 6 `визуальная иерархия и статусы`: warning-сигнал задач перенесён из колонки `Ст.` в колонку `Задача`.
