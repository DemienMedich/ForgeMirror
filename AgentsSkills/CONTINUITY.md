...
- State: Stage 33 pushed as 5693925. Stage 34 adds guarded permanent deletion of empty archived profiles with crash-safe file recovery; full verification passed. Stable branches unchanged.
...
- Now: Stage 34 ready for commit/push: permanent profile deletion requires an empty archived profile, blocks task/history/progress references, journals both INI locations and achievements, and exercises the real profile-manager confirmation path. Full build, smoke_qt/smoke_core, native visual QA and packaged startup without Qt on PATH passed with empty stderr.
