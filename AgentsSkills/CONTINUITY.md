...
- State: Stage 30 pushed as 1e4cd73. Stage 31 adds an employee metrics view with resolved profile names to Qt Statistics; full verification passed. Stable branches unchanged.
...
- Now: Stage 31 ready for commit/push: Statistics switches compactly between project and employee metrics, resolves current profile names without hiding historical IDs, and retains the shared search/export flow. Full build, smoke_qt/smoke_core, native visual QA and packaged startup without Qt on PATH passed with empty stderr.
