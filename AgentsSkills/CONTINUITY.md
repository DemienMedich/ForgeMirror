...
- State: Stage 7 guided task transitions implemented and verified; full migration remains in progress.
...
- Now: Stage 7 verified: nextIds-based task transitions, readiness confirmation, stale/invalid/completed transition guards, existing transactional audit/rollback. smoke_qt and smoke_core pass; dialog inspected at 480x360; native tests and packaged startup without Qt on PATH exit 0 with empty stderr. Commit/push to codex/qt-gui.
- Next: Continue Qt migration: personal login/access policies, destructive operations with rollback, skill profession assignment, achievements, settings, cloud, Pomodoro, vault and 3D. Legacy catalog category+profession serialization is lossy; Qt rejects unsafe round trips instead of changing the shared format.
