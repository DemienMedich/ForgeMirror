...
- State:
  - Ветка: beta. Версия: 0.4.35.
  - Последнее изменение: форма постановки задач делает создание проекта явной частью сценария `задача -> проект` — рядом с выбором проекта появилась отдельная CTA `Новый проект`, inline-редактор стал заметнее и получил явную отмену.
- Done:
