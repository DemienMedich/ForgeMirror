...
- State: Stage 12 pushed as f6a6a15. Stage 13 personal wallet scenario implemented; verified. Stable branches unchanged.
...
- Now: Stage 13 ready for commit/push: wallet metric and personal-session-only Evil Spirit removal for 200 coins through the existing profile+vault rollback service. Confirmation defaults to No; pending recovery blocks mutation. smoke_qt/smoke_core and native Windows tests pass; screen inspected. Rebuilt package starts without installed Qt on PATH, exit 0 and empty stderr.
- Next: Continue Qt migration: Pomodoro actions, persistent trusted-device access and profile audit, destructive operations with rollback, settings, cloud, vault and 3D. Existing malformed cross-file IDs are not rewritten automatically; unknown references are preserved for review.
