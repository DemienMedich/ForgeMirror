...
- State: Stage 8 session-only profile access implemented and verified; public read-only browsing remains consistent with ImGui.
...
- Now: Stage 8 verified: selected-profile password login/logout, in-memory credential fingerprint, revocation on selection/credential/block/archive checks, own-password session guard, no admin elevation. smoke_qt and smoke_core pass; login form inspected; native tests and packaged startup without Qt on PATH exit 0 with empty stderr. Commit/push to codex/qt-gui.
- Next: Continue Qt migration: profession CRUD/skill assignment, achievements and personal wallet/Pomodoro actions, persistent trusted-device access and profile audit, destructive operations with rollback, settings, cloud, vault and 3D. Legacy catalog category+profession serialization is lossy; Qt rejects unsafe round trips instead of changing the shared format.
