...
- State: Stage 27 pushed as d3b0427. Stage 28 adds versioned before/after rollback envelopes to new Qt XP awards; full verification passed. Stable branches unchanged.
...
- Now: Stage 28 ready for commit/push: new Qt awards retain the pre-task snapshot plus an exact postcondition, including a verified tasks.json round trip. Later profile progress fails the postcondition instead of being silently erased by stale rollback. Full build, smoke_qt/smoke_core, native smoke and packaged startup without Qt on PATH passed with empty stderr.
