...
- State: Stage 28 pushed as 5c9eb13. Stage 29 adds guarded transactional deletion of current Qt-v2 awarded tasks with participant-profile rollback; full verification passed. Stable branches unchanged.
...
- Now: Stage 29 ready for commit/push: awarded deletion requires semantic postcondition matches for every participant, journals all profile/task/audit files, and restores byte-identical post-award profile data on injected audit failure. The real UI path restores the pre-award profile on success. Full build, smoke_qt/smoke_core, native visual QA and packaged startup without Qt on PATH passed with empty stderr.
