...
- State: Stage 26 pushed as 58f18c8. Stage 27 adds confirmed restart-safe deletion for tasks without awarded XP; full verification passed. Stable branches unchanged.
...
- Now: Stage 27 ready for commit/push: administrators can delete an unambiguous task without participants through a confirmed metadata transaction. Awarded tasks remain blocked until every participant profile can join the recovery journal. Full build, smoke_qt/smoke_core, native visual QA and packaged startup without Qt on PATH passed with empty stderr.
