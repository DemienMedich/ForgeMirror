...
- State: Stage 24 pushed as 0557f64. Stage 25 adds restart-safe task creation across tasks, task audit and last-good backup; full verification passed. Stable branches unchanged.
...
- Now: Stage 25 ready for commit/push: the actual Qt task-creation path uses the metadata recovery journal and restores both memory and exact file bytes when the audit append fails after the task save. Full build, smoke_qt/smoke_core, native visual smoke and packaged startup without Qt on PATH passed with empty stderr.
