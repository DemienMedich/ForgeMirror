...
- State: Stage 25 pushed as 883406a. Stage 26 adds restart-safe ordinary task-status transitions across tasks, task audit and last-good backup; full verification passed. Stable branches unchanged.
...
- Now: Stage 26 ready for commit/push: the actual Qt non-XP status path uses the metadata recovery journal and restores both memory and exact file bytes when audit append fails after the task save. The compact status dialog is fully localized. Full build, smoke_qt/smoke_core, native visual QA and packaged startup without Qt on PATH passed with empty stderr.
