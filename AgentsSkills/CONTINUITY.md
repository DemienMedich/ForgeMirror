...
- State: Stage 6 pipeline definition editor implemented and verified; full migration remains in progress.
...
- Now: Stage 6 verified: pipeline creation/editing with stable IDs, complete metadata, next-step links, preservation of missing links, one atomic primary save and current task-row titles. smoke_qt and smoke_core pass; all four tabs inspected at 520x440; native tests and packaged startup with no Qt on PATH exit 0, stderr empty. Commit/push to codex/qt-gui.
- Next: Continue Qt migration: guided pipeline transitions, personal login/access policies, destructive operations with rollback, skill profession assignment, achievements, settings, cloud, Pomodoro, vault and 3D. Legacy catalog category+profession serialization is lossy; Qt rejects unsafe round trips instead of changing the shared format.
